...
// In-memory cart storage (plain object, no Object.create(null))
const cart = {};
...
app.post('/cart/:sessionId', (req, res) => {
  ...
  // No validation on itemId or quantity besides existence

  if (!cart[sessionId]) {
    cart[sessionId] = {}; // Plain object
  }
...
app.listen(PORT, () => {
  console.log(`Vulnerable cart server running on http://localhost:${PORT}`);
});
