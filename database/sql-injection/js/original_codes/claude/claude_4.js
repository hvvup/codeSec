// Implementation 4: Login with Middleware and Query Builder Pattern
...
// Simple query builder for authentication
const AuthQueries = {
...
function validateLoginInput(req, res, next) {
  ...
  // Basic validation to prevent SQL injection attempts (in addition to parameterized queries)
  if (typeof username !== 'string' || typeof password !== 'string') {
    return res.status(400).json({ error: 'Invalid input format' });
  }
  
  // You could add more validation here (e.g., length, allowed characters, etc.)
  
  next(); // Proceed to the next middleware/route handler
}

// Login route with middleware and query builder pattern
app.post('/api/login', validateLoginInput, (req, res) => {
  const { username, password } = req.body;
  
  const query = AuthQueries.findByCredentials(username, password);
  
  db.get(query.sql, query.params, (err, user) => {
    if (err) {
