...
app.get('/messages/from', (req, res) => {
  try {
    ...
    // SAFE: Use variable substitution to prevent XPath Injection
    const compiledXPath = xpath.parse("//message[sender/text()=$sender]");
    const messages = compiledXPath.select({
      node: xmlDoc,
    ...
    const result = messages.map((messageNode) => {
      const content = xpath.select1('string(content/text())', messageNode);
      const timestamp = xpath.select1('string(timestamp/text())', messageNode);
      return {
        sender: sender,
        content: content,
        timestamp: timestamp
