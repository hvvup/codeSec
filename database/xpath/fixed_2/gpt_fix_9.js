...
app.get('/movies/rating', validateMovieTitle, (req, res) => {
    try {
        ...
        // SECURE: Use variable substitution to avoid XPath injection
        const query = xpath.parse(`/movies/movie[title=$movieTitle]/rating/text()`);
        const result = query.select({
            node: moviesDoc,
            variables: { movieTitle: movieTitle }
        });
