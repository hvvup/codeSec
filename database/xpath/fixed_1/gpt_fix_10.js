...
// ✅ Input validation for orderId format
function isValidOrderId(orderId) {
  return /^ORD-\d{4}$/.test(orderId);
}

// Secure findOrderById function with validation & escaping
const findOrderById = (doc, orderId) => {
  try {
    ...
    const safeOrderId = xpathLiteral(orderId);
    const orderNode = select(`/orders/order[id=${safeOrderId}]`, doc)[0];
...
app.get('/order/detail', (req, res) => {
  ...
  if (!xmlDoc) {
    return res.status(500).json({ error: 'Failed to process orders data' });
  }
