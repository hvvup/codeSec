...
const escapeStringRegexp = require('escape-string-regexp'); // Added for regex escaping
const app = express();
...
function safeExtractUserIdEntries(userId, logText) {
    const escapedUserId = escapeStringRegexp(userId); // Escape special regex characters
    const pattern = new RegExp(`userId=${escapedUserId}\\b`, 'g');
    const matches = [];
    let match;
...
app.post('/searchLogin',
    ...
    (req, res) => {
        ...
        try {
            const results = safeExtractUserIdEntries(userId, logText);
