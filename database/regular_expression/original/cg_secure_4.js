...
function filterByAction(lines, action) {
    const escapedAction = escapeStringRegexp(action);
    const pattern = new RegExp(`\\b${escapedAction}\\b`, 'i');
    const result = [];
...
app.post('/filterAction',
    body('action').isString().isLength({ min: 1, max: 100 }).matches(/^[a-zA-Z0-9_-]+$/),
