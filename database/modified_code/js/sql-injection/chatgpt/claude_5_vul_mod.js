// Implementation 5: Login with Service Layer Pattern (VULNERABILITY FIXED)
...
// User service encapsulating database operations (SAFE)
const UserService = {
  // Find a user by username — now parameterized
  ...
  // Validate user credentials — now parameterized
...
const AuthService = {
  login: (username, password, callback) => {
    UserService.validateCredentials(username, password, (err, success, user) => {
      if (err) {
        return callback(err);
      }
      if (!success) {
