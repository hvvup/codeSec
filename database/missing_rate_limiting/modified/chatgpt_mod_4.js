...
// ✅ Rate limiter middleware to prevent abuse of PDF generation
const pdfRateLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 10, // max 10 requests per IP per 15 mins
  message: { error: 'Too many PDF generation requests. Please try again later.' },
  standardHeaders: true,
  legacyHeaders: false,
});

app.post('/generate-pdf', pdfRateLimiter, (req, res) => { // ✅ applied rate limiter here
  const { text } = req.body;
