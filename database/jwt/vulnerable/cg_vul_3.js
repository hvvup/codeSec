...
function authenticateToken(req, res, next) {
  ...
  // Decode header to check if "none" algorithm is used
  const header = JSON.parse(Buffer.from(token.split('.')[0], 'base64url').toString());
  const isNoneAlgorithm = header.alg === 'none';

  // Vulnerability: skip signature verification if alg is "none"
  const key = isNoneAlgorithm ? null : SECRET_KEY;
...
app.listen(PORT, () => {
  console.log(`Insecure file API running on http://localhost:${PORT}`);
});
