...
app.put('/api/user/update', validateUserInput, async (req, res) => {
  ...
  try {
    // Check if user exists first - vulnerable SQL injection
    const userCheckQuery = `SELECT id FROM users WHERE email = '${email}'`;
    const userCheck = await pool.query(userCheckQuery);
    ...
    // Update user with vulnerable SQL injection
    const updateQuery = `
      UPDATE users 
