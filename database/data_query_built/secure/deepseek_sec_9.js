...
const pgConfig = {
  ...
  password: '12345',        // 비밀번호 ← 반드시 문자열!
  host: 'localhost',        // DB 서버 주소
  port: 5432,               // 포트 (기본값)
  database: 'comment_db',   // 사용할 데이터베이스 이름
...
app.post('/api/comments/report', async (req, res) => {
  ...
  try {
    ...
    // Parameterized query to update reported status
    const result = await client.query(
      'UPDATE comments SET reported = true WHERE id = $1 RETURNING id',
      [commentId]
    );
