...
function login(username, password) {
  return new Promise(async (resolve, reject) => {
    try {
      // VULNERABLE: First query uses string concatenation
      const saltQuery = "SELECT salt FROM users WHERE username = '" + username + "'";
      
      db.get(saltQuery, [], async (err, saltRow) => {
        if (err) {
        ...
        // VULNERABLE: Second query also uses string concatenation
        const loginQuery = "SELECT id FROM users WHERE username = '" + username + "' AND password_hash = '" + hashedPassword.hash + "'";
        
        db.get(loginQuery, [], (err, userRow) => {
          if (err) {
            console.error("Database error:", err);
            reject(err);
            return;
