...
const dbHelper = {
  ...
  findUserById: (id) => {
    return new Promise((resolve, reject) => {
      // SQL Injection vulnerability - using string concatenation
      const query = `SELECT id, username, email, created_at FROM users WHERE id = ${id}`;
      db.get(query, (err, row) => {
        if (err) return reject(err);
        resolve(row);
      });
  ...
  // VULNERABLE: Using string concatenation for token insertion
  createToken: (userId, accessToken, refreshToken, expiresAt) => {
    return new Promise((resolve, reject) => {
      // SQL Injection vulnerability - using string concatenation
      const query = `INSERT INTO oauth_tokens (user_id, access_token, refresh_token, expires_at) 
  ...
  // VULNERABLE: Using string concatenation for token lookup
  findTokenByAccessToken: (token) => {
    return new Promise((resolve, reject) => {
      // SQL Injection vulnerability - using string concatenation
      const query = `SELECT * FROM oauth_tokens WHERE access_token = '${token}'`;
      db.get(query, (err, row) => {
        if (err) return reject(err);
        resolve(row);
      });
  ...
  // VULNERABLE: Using string concatenation for token deletion
  deleteToken: (token) => {
    return new Promise((resolve, reject) => {
      // SQL Injection vulnerability - using string concatenation
      const query = `DELETE FROM oauth_tokens WHERE access_token = '${token}'`;
      db.run(query, function (err) {
        if (err) return reject(err);
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Store tokens in database - vulnerable to SQL injection
...
app.get('/api/users/search', authenticate, async (req, res) => {
  try {
    ...
    // SQL Injection vulnerability - using string concatenation
    return new Promise((resolve, reject) => {
      const query = `SELECT id, username, email, created_at FROM users WHERE username LIKE '%${username}%'`;
...
app.post('/api/logout', async (req, res) => {
  try {
    ...
    // Delete token from database - vulnerable to SQL injection
...
const authenticate = async (req, res, next) => {
  try {
    ...
    // Find token in database - vulnerable to SQL injection
    ...
    // Attach user to request - vulnerable to SQL injection
...
app.post('/api/refresh-token', async (req, res) => {
  try {
    ...
    // Find token in database - VULNERABLE: Using string concatenation
    const tokenData = await new Promise((resolve, reject) => {
      const query = `SELECT * FROM oauth_tokens WHERE refresh_token = '${refresh_token}'`;
    ...
    // Delete old token - vulnerable to SQL injection
    ...
    // Store new tokens in database - vulnerable to SQL injection
