...
const dbHelper = {
  ...
  // Verify token - VULNERABLE TO SQL INJECTION
  verifyToken: async (token) => {
    // VULNERABILITY: Direct string concatenation in SQL query
    const query = 'SELECT t.*, u.id as user_id, u.username, u.email FROM tokens t ' +
  ...
  // Check if IP is blocked - VULNERABLE TO SQL INJECTION
  isIpBlocked: async (ipAddress) => {
    // VULNERABILITY: Direct string concatenation in SQL query
    const query = 'SELECT COUNT(*) as count FROM login_history ' +
      'WHERE ip_address = "' + ipAddress + '" AND success = false AND attempt_time > DATE_SUB(NOW(), INTERVAL 15 MINUTE)';
...
// Protected route example - VULNERABLE TO SQL INJECTION
app.get('/api/profile', auth, async (req, res) => {
  try {
    // VULNERABILITY: User ID from request used directly in query string
    const [user] = await pool.query('SELECT id, username, email, created_at FROM users WHERE id = ' + req.user.id);
    res.json(user[0]);
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ message: 'Server error' });
