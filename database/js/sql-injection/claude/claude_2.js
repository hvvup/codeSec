...
const userQueries = {
  ...
  findById: (id) => {
    return pool.query('SELECT id, username, email, created_at FROM users WHERE id = $1', [id]);
  }
};
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO sessions (user_id, session_id, expires) VALUES ($1, $2, $3)',
      [user.id, req.session.sessionId, expiryDate]
    );
  ...
  try {
    if (req.session) {
      ...
      if (req.session.sessionId) {
        await pool.query('DELETE FROM sessions WHERE session_id = $1', [req.session.sessionId]);
      }
...
const isAuthenticated = async (req, res, next) => {
  ...
  try {
    ...
    const sessionResult = await pool.query(
      'SELECT * FROM sessions WHERE user_id = $1 AND session_id = $2 AND expires > NOW()',
